...
import { boundedSin } from 'bounded-sine';
...
export class GameAudio {
  ...
  renderReverbTail(callback) {
    ...
    const tailHPFilter = this.createFilter(offline, 'highpass', 500, tailLPFilter);
  ...
  intervalToHz(interval) {
    ...
    return this.baseNote * Math.pow(Math.pow(baseExp, 1 / 12), Math.abs(interval));
  }
  ...
  update(gameTime) {
    if (this.state === 'started') {
      ...
      this.setParam(this.premasterFilter.frequency, this.premasterFilterFreq(elapsed));
      const divisions = {
